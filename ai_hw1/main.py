...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    list_weights = np.linspace(low_heuristic_weight, high_heuristic_weight, n)
    list_costs = []
    list_states = []
    ...
    plot_distance_and_expanded_wrt_weight_figure(problem.name, list_weights_with_solution, list_costs, list_states)
...
def toy_map_problem_experiments():
    ...
    astar_air_dist = AStar(AirDistHeuristic)
    res = astar_air_dist.solve_problem(toy_map_problem)
    print(res)
...
def mda_problem_with_astar_experiments():
    ...
    astar_max_air_dist = AStar(MDAMaxAirDistHeuristic)
...
def multiple_objectives_mda_problem_experiments():
    ...
    astar_tests_travel_dist = AStar(MDATestsTravelDistToNearestLabHeuristic)
    res = astar_tests_travel_dist.solve_problem(moderate_mda_problem_with_tests_travel_dist_cost)
    print(res)
    ...
    optimal_distance_cost = astar_mst.solve_problem(moderate_mda_problem_with_distance_cost).solution_g_cost
    epsilon = 0.6
    max_distance_cost = (1 + epsilon) * optimal_distance_cost
    astar_nearest_lab = AStar(MDATestsTravelDistToNearestLabHeuristic,
                              open_criterion=lambda search_node:
                              search_node.cost.distance_cost <= max_distance_cost)
