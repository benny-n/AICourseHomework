...
class MDAMSTAirDistHeuristic(HeuristicFunction):
    ...
    def _calculate_junctions_mst_weight_using_air_distance(self, junctions: List[Junction]) -> float:
        ...
        G.add_weighted_edges_from([(j1, j2,
                                    self.cached_air_distance_calculator.get_air_distance_between_junctions(j1, j2))
                                   for j1 in junctions for j2 in junctions if j1 != j2])
...
class MDATestsTravelDistToNearestLabHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        cost = sum([apartment.nr_roommates * air_dist_to_closest_lab(apartment.location)
                    for apartment in apartments_to_visit])
        tests_taken = state.get_total_nr_tests_taken_and_stored_on_ambulance()
        if tests_taken > 0:
            return cost + air_dist_to_closest_lab(state.current_location) * tests_taken
        else:
            return cost
